...
#define NVIC_MMAR    (*(volatile unsigned int*)  (0xE000ED34u))  // Memory Manage Address Register
#define NVIC_CFSR    (*(volatile unsigned int*)  (0xE000ED28u))  // Configurable Fault Status (MFSR|BFSR|UFSR)
#define NVIC_SCB_CCR (*(volatile unsigned int*)  (0xE000ED14u))  // SCB CCR?bit3=UNALIGN_TRP?
...
static void HF_UartPutc(char c)
{
  USART1->TDR = (uint8_t)c;
  while ((USART1->ISR & USART_ISR_TC) == 0) { }
}

static void HF_UartPuts(const char *s)
{
  if (s == 0) {
    return;
  }
...
static void HF_UartHex32(unsigned int v)
{
  static const char d[] = "0123456789ABCDEF";
  unsigned int i;
  for (i = 0u; i < 8u; i++) {
    unsigned int sh = 28u - (i * 4u);
    HF_UartPutc(d[(v >> sh) & 0xFu]);
  }
}
...
void HardFaultHandler(unsigned int* pStack) {
  ...
  HardFaultRegs.mfsr.byte       = NVIC_MFSR;    // Memory Management Fault Status Register
  ...
  {
    unsigned int cfsr = NVIC_CFSR;
    unsigned int mmar = NVIC_MMAR;
    uint8_t i;
    const char *pError = ERR_INFO;

    for (i = 0; i < (uint8_t)strlen(ERR_INFO); i++) {
      HF_UartPutc(pError[i]);
    }
    HF_UartPuts("HF PC=0x");
    HF_UartHex32(HardFaultRegs.SavedRegs.pc);
    HF_UartPuts(" LR=0x");
    HF_UartHex32(HardFaultRegs.SavedRegs.lr);
    HF_UartPuts(" xPSR=0x");
    HF_UartHex32(HardFaultRegs.SavedRegs.psr.byte);
    HF_UartPuts("\r\nHF HFSR=0x");
    HF_UartHex32(HardFaultRegs.hfsr.byte);
    HF_UartPuts(" CFSR=0x");
    HF_UartHex32(cfsr);
    HF_UartPuts(" BFAR=0x");
    HF_UartHex32(HardFaultRegs.bfar);
    HF_UartPuts(" MMFAR=0x");
    HF_UartHex32(mmar);
    HF_UartPuts(" CCR=0x");
    HF_UartHex32(NVIC_SCB_CCR);
    HF_UartPuts(" (bit3=UNALIGN_TRP)\r\n");
  }
