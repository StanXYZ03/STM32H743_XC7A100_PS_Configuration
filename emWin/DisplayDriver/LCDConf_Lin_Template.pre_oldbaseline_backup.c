...
static uint32_t LCDConf_GetLineEventLine(void)
{
    return (((hltdc.Instance->AWCR & LTDC_AWCR_AAH) >> 16U) + 1U);
}
...
static void _EnableLTDCLineIRQ(void)
{
	...
	/* STM32H7 RM0433: LIPCR=0 is explicitly invalid ("cannot be positioned on
	   ...
	   before the next frame's active scan begins. */
...
static void LCD_LL_Init(void) 
{
	...
	/* LTDC line interrupt is armed on demand when a frame swap is pending. */
